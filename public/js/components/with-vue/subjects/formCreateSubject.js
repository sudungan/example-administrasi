...
export default defineComponent({
    ...
    emit: ['backTo'],
    setup(props, {emit}) {
        const  subject = reactive({ name: '', teacher_id: '', classroom_id: '', colour: '', total_jp: '' })
        const  errors = reactive({ name: '', teacher_id: '', classroom_id: '', colour: '', total_jp: '' })
        ...
        async function storeSubject() {
            try {
                ...
                let result = await axios.post(`store-subject`)
            } catch (error) {

            }
        }
        return {
            subject, closeCreateForm, storeSubject, errors, listTeacher, isLoading
        }
    },
    template: `
