...
<div class="relative overflow-x-auto shadow-md sm:rounded-lg">
    <table border="1" class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        <thead class="text-xs text-white uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
            <tr>
                <th rowspan="2"  @click="btnAddTime" class="px-2 py-2 text-start border-r border-gray-300">
                    <div class="flex flex-col items-start">
                        <span class="underline cursor-pointer hover:font-bold hover:text-red-900">Time</span>
                        <span>Kelas</span>
                    </div>
                </th>
                <th v-for="day in days" :colspan="childListClassroom.length" :key="day"> {{ day }} </th>
            </tr>
            <tr>
                <template v-for="day in days" :key="day">
                    <th v-for="classroom in classrooms" :key="day + classroom">{{ classroom }}</th>
                </template>
            </tr>
