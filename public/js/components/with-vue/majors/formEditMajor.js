...
export default defineComponent({
    name: 'formEditMajor', // nama child component
    props: {
        visableCard: {
            type: String,
            required: true
    ...
    emits: ['backTo', 'reload'],
    setup(props, {emit}) {
        const editMajor = reactive({ id: '', name: '',  user_id: '' }) // tempat menampung data edit major
        const errors = reactive({  name: '',  user_id: '' })
        const chiidMajor = ref({...props.major}) // melakukan clone data ke chiidMajor
        ...
        watch(() => props.major, (newVal) => { Object.assign(editMajor, newVal)  }, { immediate: true });
        ...
        async function updateMajor(majorId) {
            try {
                let isValid = true;
                childIsLoading.value = true;

                for (let key in editMajor) {
                    if (!editMajor[key].toString().trim()) {
                ...
                let sendUpdateMajor = {
                    id: editMajor.id,
                    name: editMajor.name,
                    user_id: editMajor.user_id,
                    addition_role_id: childHeadMajorId.value
                }
                childIsLoading.value = true;
                let result = await axios.put(`/update-major/${editMajor.id}`, sendUpdateMajor)
        ...
        return {
            updateMajor, btnCloseEdit, errors, editMajor,
            fieldLabels, major: chiidMajor, teachers: childTeachers,
        ...
        <div class="relative p-4 w-full max-w-2xl max-h-full">
            <div class="relative bg-gray-300 rounded-lg shadow-sm dark:bg-gray-900">
                ...
                <div class="p-4 md:p-5 space-y-4">
                    <form @submit.prevent="updateMajor(editMajor.id)" class="space-y-4">
                        <div class="grid gap-2 mb-2 grid-cols-2">
                            ...
                            <div class="col-span-2 sm:col-span-1">
                                ...
                                <select v-model="editMajor.user_id" id="user_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:gray-600 dark:focus:ring-primary-500 dark:focus:border-primary-500">
                            ...
                            <div class="relative flex mt-2 gap-2">
                                <button
                                    ...
                                    <p v-if="waitingProcess"> process... </p>
                                    <p v-else> Update</p>
                                </button>
                                <button
                                    :disabled="waitingProcess"
