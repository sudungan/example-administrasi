...
export default defineComponent({
    ...
    setup(props, {emit}) {
        ...
        async function btnEditClassroom(classroomId) {
            emit('edit', classroomId)
        }

        async function btnDeleteConfirmation(classroomId) {
            confirmDelete('Yakin dihapus?', async (result)=>{
                if(!result.isConfirmed) {
                    return
                }
                await swalLoading('Menghapus data kelas..',async (result)=> {
                    try {
                        let result = await axios.delete(`/delete-classroom-by/${classroomId}`)
...
<div class="relative overflow-x-auto shadow-md sm:rounded-lg">
    ...
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        ...
        <tbody>
             <template v-for="(classroom, index) in dataProvide" :key="index">
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
                    ...
                    <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                        ...
                        <a @click="btnEditClassroom(classroom.id)"
                        ...
                        <a @click="btnDeleteConfirmation(classroom.id)"
    ...
    <template v-if="childListClassroom.length > 0">
