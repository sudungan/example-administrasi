...
>
    ...
    <div class="flex gap-4">
        ...
        <livewire:users.form-create-user-detail :userId="$userId"/>
            {{-- <div x-transition.duration.500ms class="flex bg-gray-400 fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
            </div> --}}
        @endif
    @if (!$edit)
                            ...
                            <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                             ...
                             <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                ...
                                @if (isset($colors[$role]))
                                    <span class="bg-{{ $colors[$role] }}-100 text-{{ $colors[$role] }}-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded-sm dark:bg-gray-700 dark:text-{{ $colors[$role] }}-400 border border-{{ $colors[$role] }}-400">
                                        {{ $role }}
                                    </span>
                                @endif
                            </th>
                            <td class="px-6 py-4 flex gap-3">
                                <a x-on:click="$wire.editRole({{$user->id}})"  data-tooltip-target="tooltip-show-{{$user->id}}" class="cursor-pointer font-medium text-blue-600 dark:text-blue-500 hover:underline">
